# CS Engineering - Assignment Repository

Welcome! This repository contains all the submissions for the semester. Per the submission guidelines, each assignment is hosted on its own dedicated Git branch. 

---

## Navigation Guide

| Assignment | Description | Language / Framework | Branch Link |
| :--- | :--- | :--- | :--- |
| **Assignment 1** | Basic Problem Solving (Arrays & Strings) | Python, C++ | [View Branch](../../tree/assignment-1) |
| **Assignment 2** | Portfolio & Dashboard UI | React.js (Vite) | [View Branch](../../tree/assignment-2) |
| **Assignment 3** | Mobile Coffee Shop App | React Native (Expo) | [View Branch](../../tree/assignment-3) |

---

## How to Review and Run

To check out a specific assignment locally, clone the repo and switch to the respective branch:

```bash
# Clone the repository
git clone [https://github.com/YOUR_USERNAME/YOUR_REPO_NAME.git](https://github.com/YOUR_USERNAME/YOUR_REPO_NAME.git)
cd YOUR_REPO_NAME

# Switch to the assignment you want to test
git checkout assignment-3